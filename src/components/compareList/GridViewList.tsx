...
interface Props {
  ...
  handleAddCardButton: any;
  handleDeleteCardButton: any;
  isAuth: boolean;
}
...
const GridViewList = ({
  ...
  useEffect(() => {
    console.log("attributeNames changed: ", attributeNames);
  }, [attributeNames]);
  ...
  return attributeNames.length > 0 ? (
    ...
    >
      <ul className="mb-[1rem] hidden sm:flex sm:flex-col" id="attribute-names">
        ...
        {attributeNames.map((attributeObj: any, index: number) => (
          <li
